...
export function Navbar() {
  ...
  return (
    ...
    <nav className="bg-background border-b border-border sticky top-0 z-50 shadow-sm">
      <div className={`max-w-7xl mx-auto ${language === 'en' ? 'px-6 sm:px-8 lg:px-10' : 'px-4 sm:px-6 lg:px-8'}`}>
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link href="/" className="flex items-center gap-2 group">
              ...
              <div className="hidden sm:flex flex-col">
                <span className="text-sm font-bold text-primary group-hover:text-accent transition-colors font-serif">
                  {t('org.name')}
                </span>
              </div>
            </Link>
          ...
          {/* Desktop Navigation */}
          <div className={`hidden xl:flex items-center ${language === 'en' ? 'gap-0.5' : 'gap-1'}`}>
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className={`py-2 font-medium text-foreground hover:text-primary hover:bg-accent/10 rounded-md transition-colors ${
                  language === 'en' ? 'px-1.5 text-[13px]' : 'px-2.5 text-sm'
                }`}
              >
                {link.label}
              </Link>
